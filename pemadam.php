...
// Memeriksa sesi pengguna untuk pemadam
if (isset($_SESSION['user_id']) && $_SESSION['role'] == 'pemadam') {
    $user_id = $_SESSION['user_id'];

    $result = $conn->query("SELECT status FROM device_status WHERE id = 1");
$status_alat = $result->fetch_assoc()['status'];

$result = $conn->query("SELECT status, battery_level FROM device_status WHERE id = 1");
$device_data = $result->fetch_assoc();
$status_alat = $device_data['status'];
$battery_level = $device_data['battery_level'];


    // Ambil waktu aktivitas terakhir dari database
    $sql = "SELECT last_activity FROM aktif_akun WHERE user_id = ? AND role = 'pemadam'";
    ...
    // Jika selisih waktu lebih dari 30 detik, logout otomatis
    if ($time_difference > 30) {
        $delete_sql = "DELETE FROM aktif_akun WHERE user_id = ? AND role = 'pemadam'";
...
// Ambil daftar trainer yang belum disetujui aksesnya
$trainer_query = "SELECT ta.id, u.username, ta.approved_by_pemadam 
                  FROM trainer_access ta 
                  JOIN users u ON ta.trainer_id = u.id 
                  WHERE ta.approved_by_pemadam = 0";
$trainer_result = $conn->query($trainer_query);

// Ambil status alat dari database
$result = $conn->query("SELECT status, battery_level, water_level FROM device_status WHERE id = 1");
$device_data = $result->fetch_assoc();
$status_alat = $device_data['status'];
$battery_level = $device_data['battery_level'];
...
<div class="col-md-6 col-lg-4">
    ...
    <!-- Daftar Permintaan Akses Trainer -->
    <div class="container mt-4">
        <h3>Daftar Permintaan Akses Trainer</h3>
        <div class="table-responsive">
            <table class="table table-hover">
                <thead>
                    <tr>
                        <th>Username Trainer</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($trainer_result->num_rows > 0): ?>
                        <?php while ($trainer_row = $trainer_result->fetch_assoc()): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($trainer_row['username']); ?></td>
                                <td>
                                <form method="POST" action="approve_access.php" style="display:inline;">
                                    <input type="hidden" name="trainer_access_id" value="<?php echo $trainer_row['id']; ?>">
                                    <input type="hidden" name="action" value="approve">
                                    <button type="submit" class="btn btn-success btn-sm">Setujui Akses</button>
                                </form>
                                <!-- Form Tolak Akses -->
                                <form method="POST" action="approve_access.php" style="display:inline;">
                                    <input type="hidden" name="trainer_access_id" value="<?php echo $trainer_row['id']; ?>">
                                    <input type="hidden" name="action" value="reject">
                                    <button type="submit" class="btn btn-danger btn-sm">Tolak Akses</button>
                                </form>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="2">Tidak ada permintaan persetujuan akses.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
// Fungsi untuk memperbarui waktu aktivitas ke server
...
// Reset timeout jika pengguna melakukan aktivitas
function resetActivityTimeout() {
    clearTimeout(activityTimeout);
    activityTimeout = setTimeout(updateActivity, 10000);  // Setiap 10 detik jika ada aktivitas
}

// Menyimak aktivitas pengguna
window.onload = resetActivityTimeout;
window.onmousemove = resetActivityTimeout;
window.onkeydown = resetActivityTimeout;
</script>
